...
(
  ...
  sleep 5
  LOG_NAME="block-rootful-pfd-$DATE.log"
  rm -f $LOG_NAME
  for BLOCK_SIZE in ${BLOCK_SIZES[@]}
  do
    ssh $VM2 "sudo nerdctl exec block-client /bench -count 1 -thread-num 1 -url http://$VM1_ADDR:8080/blk-$BLOCK_SIZE"
    ssh $VM2 "sudo nerdctl exec block-client /bench -count $COUNT -thread-num $THREAD_NUM -url http://$VM1_ADDR:8080/blk-$BLOCK_SIZE" >> $LOG_NAME
  done
