...
(
  ...
  ssh $VM1 "sudo nerdctl run -d --name iperf3-server $IMAGE_NAME iperf3 -s"
  ssh $VM2 "sudo nerdctl run -d --name iperf3-client $IMAGE_NAME"
  ...
  sleep 1
  set +e
  #ssh $VM2 "sudo nerdctl exec iperf3-client iperf3 -c $VM1_VXLAN_ADDR -i 0 -t $TIME --connect-timeout 1000 -P $PARALLEL_NUM -J -d"> iperf3-rootful-vxlan-p$PARALLEL_NUM-$DATE.log
  ssh $VM2 "sudo nerdctl exec iperf3-client iperf3 -c $VM1_VXLAN_ADDR -i 0 -t 70 --connect-timeout 1000 -P $PARALLEL_NUM -J"> iperf3-rootful-vxlan-p$PARALLEL_NUM-$DATE.log
  set -e
  #if [ $? -ne 0 ]; then
